...
process {
	...
	if (-not ($ComputerList))
	{
		try 
		{
			# Filter for all Windows Systems, ignore Windows 10
	...
	foreach($computer in $ComputerList) 
	{
		...
		{
			Write-Verbose "$computer : possibly offline or unreachable : " -nonewline
		}
		...
		try 
		{
			...
			if($hotfix) 
			{
				Write-Verbose -Message "$computer : hotfix $hotfix installed"
			}
			else 
			{
				Write-Warning -Message "$computer : missing WannaCry hotfix"
				continue
			}
		}
		catch 
		{
			Write-Warning -Message "$computer : Unable to get Hostfix Info"
			continue
		}
	}
}